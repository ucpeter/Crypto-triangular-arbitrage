...
async fn fetch_binance(_client: &Client) -> Result<Vec<PairPrice>, String> {
    info!("fetching binance via websocket snapshot");
                    ...
                    if price > 0.0 && vol > 0.0 {
                        ...
                        for q in known_quotes {
                            if symbol.ends_with(q) {
                                base = symbol.trim_end_matches(q).to_string();
                                quote = q.to_string();
                                break;
                            }
                        }
    ...
    Err("binance ws no snapshot".to_string())
}
